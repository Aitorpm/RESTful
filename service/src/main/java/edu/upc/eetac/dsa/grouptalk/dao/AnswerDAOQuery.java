...
public interface AnswerDAOQuery {
    ...
    public final static String GET_ANSWERS_BY_THEMEID = "select hex(id) as id, hex(userid) as userid, hex(themeid) as themeid, content, creation_timestamp, last_modified from answers, themes t where t.id=unhex(?) and a.themeid=t.id";
